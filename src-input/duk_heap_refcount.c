...
/* Remove 'hdr' from heap_allocated, leaving 'hdr' link pointers as garbage. */
DUK_LOCAL DUK_INLINE void duk__refcount_remove_from_heap_allocated(duk_heap *heap, duk_heaphdr *hdr) {
	duk_heaphdr *prev;
	duk_heaphdr *next;
...
DUK_LOCAL DUK_INLINE void duk__refcount_refzero_hobject(duk_heap *heap, duk_hobject *obj, duk_bool_t skip_free_pending) {
	...
	duk__refcount_remove_from_heap_allocated(heap, hdr);
...
DUK_LOCAL DUK_INLINE void duk__refcount_refzero_hbuffer(duk_heap *heap, duk_hbuffer *buf) {
	...
	duk__refcount_remove_from_heap_allocated(heap, (duk_heaphdr *) buf);
...
/* Suppress asserts:
 * - Heap thread must exist when mark-and-sweep is running.
 * - Heap thread must exist when the heap is initializing and a DECREF happens.
 * - When debugger paused -> ms_running is set.
 * - When ms_running is set, 'thr' is heap_thread because we only get called
 *   from mark-and-sweep which always runs using that thread.  This doesn't
 *   matter much now, but would allow e.g. heap->ms_running check to be done
 *   using thr->ms_running, with the assurance that thr == heap->heap_thread
 *   (so that ms_running would only need to be set for heap->heap_thread).
 */

#define DUK__RZ_SUPPRESS_ASSERT1() do { \
		DUK_ASSERT(thr != NULL); \
		DUK_ASSERT(thr->heap != NULL); \
		DUK_ASSERT(thr->heap->heap_initializing == 0 || thr->heap->heap_thread != NULL); \
