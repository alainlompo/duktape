...
DUK_LOCAL duk_ret_t duk__fake_global_finalizer(duk_context *ctx) {
	DUK_D(DUK_DPRINT("fake global torture finalizer executed"));
...
DUK_INTERNAL void duk_heap_process_finalize_list(duk_heap *heap) {
	...
	while ((curr = heap->finalize_list) != NULL) {
		...
		if (DUK_LIKELY(!heap->pf_skip_finalizers)) {
			...
			DUK_D(DUK_DPRINT("refcount after finalizer: %ld", (long) DUK_HEAPHDR_GET_REFCOUNT(curr)));
			if (DUK_HEAPHDR_GET_REFCOUNT(curr) == 0) {
#if defined(DUK_USE_DEBUG)
				if (had_zero_refcount) {
					DUK_D(DUK_DPRINT("finalized object's refcount is zero -> free immediately (refcount queued)"));
				} else {
					DUK_D(DUK_DPRINT("finalized object's refcount is zero -> free immediately (mark-and-sweep queued)"));
				}
#endif
				queue_back = 0;
			} else
		...
		if (queue_back) {
			...
			DUK_D(DUK_DPRINT("refcount finalize after finalization: %!O", curr));
			duk_hobject_refcount_finalize_norz(heap, (duk_hobject *) curr);
			duk_free_hobject(heap, (duk_hobject *) curr);
			DUK_D(DUK_DPRINT("freed hobject after finalization: %p", (void *) curr));
		}
#else
		DUK_HEAPHDR_CLEAR_FINALIZABLE(curr);
		DUK_HEAP_INSERT_INTO_HEAP_ALLOCATED(heap, curr);
...
#if defined(DUK_USE_DEBUG)
	DUK_D(DUK_DPRINT("duk_heap_process_finalize_list: %ld finalizers called", (long) count));
#endif
}
